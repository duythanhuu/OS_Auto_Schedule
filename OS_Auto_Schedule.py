...
for line in Lines:
    ...
    if str_add not in contents:
        if ("_Init</SW-SERVICE-REF>" or "_SyncIni</SW-SERVICE-REF>" or "_SyncAngular</SW-SERVICE-REF>") in str_add:
            Line_Init_formats.append(str_add)
        elif ("_Run</SW-SERVICE-REF>" or "_SyncS0</SW-SERVICE-REF>" or "_SyncS1</SW-SERVICE-REF>" or "_Sync</SW-SERVICE-REF>") in str_add:
...
if Line_Init_formats == []:
    ...
    proc_inits = "".join(Line_Init_formats)
    print(proc_inits)
    contents.insert(section[1]+10, proc_inits)
...
if Line_Run_formats == []:
    ...
    proc_runs = "".join(Line_Run_formats)
    print(proc_runs)
    contents.insert(section[3]+11, proc_runs)
